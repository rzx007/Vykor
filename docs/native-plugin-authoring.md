...
Desktop 插件页右上角“添加”菜单目前支持两个入口：

- 导入本地 Native Plugin 包：格式可以是 `.zip`、`.tar`、`.tar.gz` 或 `.tgz`；
- 从 Git 安装：输入 Git URL 和可选 branch、tag 或 commit，后台使用系统 `git` 固定到实际 commit，再进入同一条安装链路。
...
该入口不支持自动更新、独立 Repair 命令、版本回滚或旧快照垃圾回收界面，也不支持在 Agent 对话中安装、转换 Claude Code/Codex 插件、npm、归档 URL 或 Marketplace 等其他来源。请使用 CLI 的目录开发/安装流程处理这些场景。旧插件页 localStorage 配置已从页面隐藏，但保留原数据，不迁移也不删除。
