...
**效果**：下次在同一项目启动时，系统从 `facts.json` 中未命中凭据检查的事实生成 prompt 内容，不用你再说"测试服在 10.0.0.7"。每条注入的事实标出最初观察日期，并提示使用前核验；日期不是最近核验时间，系统不会仅凭时间自动删掉事实。项目事实文件要求每条记录都有来源；格式不符时停止读取并由 `/context status` 报告，不猜测旧记录的来源。
