...
**效果**：下次在同一项目启动时，系统从 `facts.json` 中有来源且未命中凭据检查的事实生成 prompt 内容，不用你再说"测试服在 10.0.0.7"。无来源或含明显凭据值的旧项目事实仍可查看，但不会进入 prompt；旧全局文件即使存在，也不再自动注入或迁入项目。

助手回复和工具输出不会成为环境事实的自动来源。`/context status` 分别报告 sourced 和 unverified 数量；sourced 只表示有来源，不表示事实已重新核验。
...
提取时只把最近 12 条消息摘要给模型，一次最多写入 3 条。自动记忆正文必须是所引用户原话的直接片段（比较时合并空白字符），不接受模型改写或推断；手动 `/remember` 不受这项限制。通过校验的记录保存来源会话 ID 和用户消息的 SHA-256 指纹，不重复保存原话。遇到同正文的旧无来源条目时，签名去重会给旧条目补来源。`/memory show` 和记忆 API 可查看来源。下一轮按当前用户输入检索相关记忆，并以临时 `system-reminder` 注入，不写进消息历史。
