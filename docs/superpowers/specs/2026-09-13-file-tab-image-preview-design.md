...
- `workspace.readFile` 已支持 PNG、JPEG、GIF、WebP、BMP 和 AVIF；Main 先校验允许根、50 MB 上限和真实文件头，再返回精确图片字节。
...
- Main 负责路径授权、大小限制、媒体类型判断和真实文件头校验；Renderer 只渲染 Main 已确认的字节。
...
| 图片超过 50 MB | `null` | 由扩展名确定的 MIME | `image_too_large` | `null` |
| 图片头不匹配 | `null` | `null` | `image_unsupported` | `null` |
...
“50 MB”在代码中固定表示 `50 * 1024 * 1024` 字节。边界规则是：
...
首版不增加图片字节缓存或 tab 淘汰策略。每个已加载图片 tab 会持有 IPC 返回的 `ArrayBuffer`，活动图片组件还会持有对应的 `Blob`。50 MB 是单文件上限，不是所有 tab 的总预算；这是本次为保持现有 tab 恢复和即时切换行为接受的内存边界。后续若实际数据表明多图片 tab 占用过高，再单独设计按 tab 释放与重新读取机制。
...
校验逻辑从 `attachment-service.ts` 抽成 `apps/desktop/src/shared/safe-image-preview.ts` 一类不依赖 Electron、Node 文件系统或 DOM 的纯函数。附件服务和 Workspace 服务共用它，避免两套允许列表或文件头规则漂移。

SVG 即使扩展名正确也不在允许列表中。HTML 或脚本内容改名为 `.png` 时必须因文件头不匹配而返回 `image_unsupported`，不得创建 `Blob` URL。
...
负责安全媒体类型、扩展名映射和文件头校验。它提供两个明确入口：

- 根据文件名返回候选安全 MIME 或 `null`，供读取前选择大小门；
- 根据候选 MIME 和字节校验文件头，通过时返回该 MIME，否则返回 `null`。

任何准备返回 `previewBytes` 的调用方都必须经过第二个入口，使其不能只信扩展名。附件服务现有的 10 MB 下载门仍留在附件服务内。
...
| 情况 | 展示 |
|---|---|
| 大于 50 MB | 标题为文件名，说明“图片超过 50 MB，无法直接预览。” |
...
2. 等于 50 MB 的合法图片可以预览；大于 50 MB 的图片不读取、不通过 IPC 传字节，并显示明确限制文案。
...
8. 对话附件仍保持原有支持格式、安全校验和 10 MB 缩略图限制。
