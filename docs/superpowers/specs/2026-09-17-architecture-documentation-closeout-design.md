...
| `docs/session-storage-design.md` | Repository、Transaction、`SessionStore` 和数据库生命周期边界 |
...
`docs/superpowers/specs/`、`docs/superpowers/plans/` 以及阶段审查、复盘材料保留原文，作为“为什么这样设计、当时如何迁移”的证据。
...
校准 `docs/session-storage-design.md`：
...
- 五份核心架构文档不存在已删除的顶层 Client 业务方法、旧协议版本或“等待下一阶段”等当前时态表述；
...
5. 只对确实会误导读者的历史文档补状态说明；
6. 运行定向搜索、文档检查、架构检查和 diff 检查；
7. 最后复读所有当前权威文档，消除重复定义和相互矛盾。
