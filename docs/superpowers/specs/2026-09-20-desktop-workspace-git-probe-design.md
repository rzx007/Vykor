...
- **activeWorkspaceIsGit**：本次新增的派生值，含义是「当前右侧面板/对话实际使用的目录是不是 git 仓库」。
...
4. **项目外会话的审阅默认停在「上一轮」。** 审阅面板现有的四个范围选项（`review-tool.tsx:45-50`）**一行不改**，「上一轮」为默认值，用户可以自行切到未提交/未暂存/已暂存。不为项目外会话加任何「禁用其他选项」的额外逻辑。

5. **缓存复用现有模式。** TTL 与去重策略参照 `renderer/src/lib/git-changes-query.ts`（`Map` + `inFlight` 去重 + `maxAgeMs`）。键使用规范化后的 `rootPath`（Windows 路径转正斜杠、去掉尾部斜杠、小写）。
...
2. hook 从 store 读取当前工作区路径（`selectActiveWorkspaceProject`）。
3. 若为项目会话 → 返回 `selectedProjectGit`，不探测。
4. 若为项目外会话 → 查模块级缓存（TTL 内直接返回），未命中则调 `git:isRepository(session.cwd)` 并写缓存。
...
| 探测缓存 | TTL + inFlight 去重 + 路径规范化 | `apps/desktop/src/renderer/src/lib/workspace-git-probe.ts` |
...
| IPC 调用抛错 | hook 捕获并置 `false`，不冒泡到错误 UI |
| 项目外会话 cwd 为空 | 直接返回 `false`，不发探测 |

## 测试

1. **`git-service` 探测测试**（`apps/desktop/src/main/features/git/`）：真实仓库目录 → `true` 且 `rootPath` 为仓库根；非仓库目录 → `false` 不抛错；空/非法路径 → `false`；子目录（仓库内嵌套）→ `true`（`--show-toplevel` 语义）。
2. **探测缓存测试**（`renderer/src/lib/workspace-git-probe.test.ts`）：TTL 内重复调用只发一次 IPC；不同路径独立缓存；失败不写缓存或按 TTL 重试（二者择一并在测试中固定行为）。
3. **hook 测试**（`renderer/src/hooks/`）：项目会话直接用 `selectedProjectGit` 且**不触发** `git:isRepository`；项目外会话 + 探测为真 → `true`；项目外 + 探测为假 → `false`；cwd 为空 → `false`。
