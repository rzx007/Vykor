...
默认推理强度在新会话未明确选择时跟随设置：每次模型请求前重新读取该会话所在项目与全局的有效配置。CLI 和 Desktop 新会话都会记录这个继承关系；会话明确选择强度后停止继承。文件暂时无效时保留该会话最近一次有效值、记录一次错误，文件恢复后继续读取。模型与供应商从最近使用的会话继承，不随设置文件变化切换。其余动态字段再按相同请求边界逐项接入。
