...
这部分属于外部能力的生命周期管理，有价值但当前优先级较低。保留以下设计备忘，暂不排入实施；恢复时再确认范围和验收标准。
...
- 空闲会话保存模型选择时立即写入现有分割线，排在后续用户消息之前；运行中切换模型不显示分割线。仅 effort 变化不新增分割线。
...
- 请求开始时仍记录实际使用的模型，供历史和子任务读取，但不补写分割线。分割线表示空闲时的手动选择，不充当模型使用记录。
...
CLI 和 Desktop 新会话对未显式设置的 `effort`、`maxTurns`、`systemPrompt` 记录默认值继承关系；每次模型请求前重新读取该会话所在项目与全局的有效配置。`workStyle`、`fastMode` 也在请求边界读取最新设置。会话显式修改前三项时停止继承对应字段；会话自定义 `systemPrompt` 保持高于设置文件说明的优先级。文件暂时无效时保留该会话最近一次有效值、记录一次错误，文件恢复后继续读取。模型与供应商从最近使用的会话继承，不随设置文件变化切换。MCP、插件和执行环境的更换另行处理。
...
8. 界面没有新增生效状态提示；错误有反馈；空闲切换立即显示分割线，运行中切换不显示。
