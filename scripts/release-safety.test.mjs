...
test("release artifacts must contain every expected file", () => {
  assert.deepEqual(
    assertArtifacts(["Vykor-1.2.3-setup.exe", "Vykor-1.2.3.AppImage"], [
      "Vykor-1.2.3.AppImage",
      "Vykor-1.2.3-setup.exe",
      "latest.yml",
    ]),
    ["Vykor-1.2.3-setup.exe", "Vykor-1.2.3.AppImage"],
  );
...
test("stable release notes contain immutable release identity and artifacts", () => {
  const notes = renderStableReleaseNotes({
    version: "1.2.3",
    commit: sha,
    artifacts: ["Vykor-1.2.3-setup.exe", "Vykor-1.2.3.AppImage"],
  });
  assert.match(notes, /Vykor 1\.2\.3/);
  assert.match(notes, new RegExp(sha));
  assert.match(notes, /Vykor-1\.2\.3-setup\.exe/);
...
test("release asset CLI preserves the version in notes and artifact validation", () => {
  const cwd = mkdtempSync(join(tmpdir(), "vykor-release-assets-"));
  ...
  try {
    ...
    assert.match(readFileSync(notes, "utf8"), /Vykor-1\.2\.3-setup\.exe/);
    assert.match(readFileSync(notes, "utf8"), /Vykor-1\.2\.3\.AppImage/);
    assert.match(readFileSync(notes, "utf8"), /Vykor-1\.2\.3\.deb/);
    ...
    writeFileSync(artifacts, "Vykor--setup.exe\nVykor-.AppImage\nVykor-.deb\nlatest.yml\nlatest-linux.yml\n");
    assert.throws(
      () => execFileSync(process.execPath, [script, "assert-artifacts", artifacts], { cwd, env, stdio: "pipe" }),
      /Command failed/,
    );
...
test("workflow preserves build, publication, verification and rerun ordering", () => {
  ...
  assert.match(workflow, /verify-clean-slate-artifacts:[\s\S]*pattern: desktop-\*[\s\S]*merge-multiple: true[\s\S]*pnpm --filter @rzx\/ohs\.\.\. build[\s\S]*pnpm --filter @vykor\/desktop build[\s\S]*pnpm check:clean-slate:artifacts/);
