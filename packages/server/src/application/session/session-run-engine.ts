...
export class SessionRunEngine {
  ...
  cancelGoalRuns(sessionId: string, goalId: string, reason: string, queuedOnly = false): string[] {
    ...
    for (const run of this.context.store.listRuns(sessionId)) {
      ...
      this.context.store.goals.markGoalContinuation(run.id, "cancelled");
    }
    return ids;
  }
  ...
  admitPromptAndMaybeRun(
    ...
    if (!input.runMetadata?.goalId) {
      const goal = this.context.store.goals.getCurrentGoal(sessionId);
  ...
  private pauseGoalForRun(runId: string | undefined): void {
    ...
    if (goal?.status === "active") this.context.store.goals.updateGoal(goal.id, { expectedRevision: goal.revision, status: "paused", reason: "用户停止了目标回合" });
  }
  ...
  private enqueueRun(
    ...
    const enqueued = this.runCoordinator.enqueue({
      ...
      work: async (workContext) => {
        ...
        const goal = isUserRun ? this.context.store.goals.getCurrentGoal(run.sessionId) :
          typeof stored.metadata.goalId === "string" ? this.context.store.goals.getGoal(stored.metadata.goalId) : undefined;
        if (!isUserRun && !goal) {
        ...
        if (goal && (goal.status === "active" || !isUserRun)) {
          ...
          const before = this.context.events.checkpoint();
          if (typeof revision !== "number" || !this.context.store.goals.startGoalRun(goal.id, revision, run.id, stored.metadata.goalRunKind === "continuation")) {
