...
import { computePluginBehaviorDigest, installLocalNativePlugin } from "@vykor/plugins";
...
beforeEach(() => {
  tempRoot = mkdtempSync(join(tmpdir(), "vk-runtime-extensions-"));
  previousConfigDir = process.env.VYKOR_CONFIG_DIR;
  process.env.VYKOR_CONFIG_DIR = join(tempRoot, "config");
});
...
function writeProjectAgentPlugin(cwd: string, suffix: string, model: string): void {
  ...
  writeFileSync(
    ...
    JSON.stringify({ schemaVersion: 1, id: `dev.vykor.${suffix}`, name: suffix, version: "1.0.0", components: { agents: ["./agents"] } }),
  );
  writeFileSync(
  ...
  store.plugins[`project:${cwd}:dev.vykor.${suffix}`] = {
    id: `dev.vykor.${suffix}`, scope: "user", enabled: true,
...
function writeProjectToolPlugin(cwd: string): void {
  ...
  writeFileSync(join(pluginDir, ".vykor-plugin", "plugin.json"), JSON.stringify({
    schemaVersion: 1,
    id: "dev.vykor.runtime-tool",
    name: "runtime-tool",
    version: "1.0.0",
...
async function spawnReviewer(
  runtime: Awaited<ReturnType<typeof createVykorRuntime>>,
  cwd: string,
) {
  ...
  await tool!.execute(
    {
      ...
      subagentType: cwd.endsWith("workspace-a")
        ? "dev.vykor.plugin-a:reviewer"
        : "dev.vykor.plugin-b:reviewer",
    },
...
describe("extension agent definition scoping", () => {
  ...
  it("keeps two live runtimes bound to the plugin agents discovered for their own cwd", async () => {
    ...
    const discoveryA = await discoverVykorExtensions(cwdA, BASE_SETTINGS);
    expect(discoveryA.pluginCapabilityInventory.agents.get("dev.vykor.plugin-a:reviewer"))
      .toEqual({ pluginId: "dev.vykor.plugin-a" });
    const runtimeA = await createVykorRuntime({
    ...
    const discoveryB = await discoverVykorExtensions(cwdB, BASE_SETTINGS);
    const runtimeB = await createVykorRuntime({
      settings: BASE_SETTINGS,
      cwd: cwdB,
      configuration: {
        client: {
...
describe("installed Native Tool activation", () => {
  it("keeps compound MCP ownership identity out of the Runtime server key", async () => {
    ...
    const discovery = await discoverVykorExtensions(cwd, BASE_SETTINGS);

    expect(discovery.pluginCapabilityInventory.mcpServers.get(
      "plugin:dev.vykor.runtime-mcp:mcp:github",
    )).toEqual({ pluginId: "dev.vykor.runtime-mcp", serverName: "github" });
  ...
  it("keeps an ambiguous installation out of the discovery inventory with a diagnostic", async () => {
    ...
    writeFileSync(storePath, JSON.stringify({
      ...
      revision: 1,
      plugins: {
        "user:first:dev.vykor.ambiguous": { ...common, cachePath: join(tempRoot, "first") },
        "user:second:dev.vykor.ambiguous": { ...common, cachePath: join(tempRoot, "second") },
      },
    }));
    ...
    expect(discovery.pluginCapabilityInventory.diagnostics).toEqual([{
      ...
      message: "Cannot choose one user installation for plugin dev.vykor.ambiguous",
      pluginId: "dev.vykor.ambiguous",
  ...
  it("skips installed plugins when the master switch is disabled", async () => {
    ...
    const disabled = await discoverVykorExtensions(cwd, {
      ...BASE_SETTINGS,
    ...
    const cannotBypassMaster = await discoverVykorExtensions(
      cwd,
    ...
    const overridden = await discoverVykorExtensions(
      cwd,
  ...
  it("does not discover plugins with unapproved permissions", async () => {
    ...
    writeFileSync(storePath, JSON.stringify(store));
    const manifestPath = join(tempRoot, "cache", "runtime-tool", ".vykor-plugin", "plugin.json");
    ...
    expect(discovery.warnings).toEqual([
      "dev.vykor.runtime-tool: missing approved plugin permissions [filesystem:write]; approve the permissions or reinstall the plugin before it can run",
    ]);
  });
  ...
  it("rejects a linked plugin whose actual manifest requests different permissions", async () => {
    ...
    expect(discovery.plugins).toEqual([]);
    expect(discovery.warnings).toEqual([
      "dev.vykor.runtime-tool: actual plugin permissions [filesystem:write] differ from the installed permission request []; reinstall the plugin",
    ]);
  });
  ...
  it("rejects a copied plugin when its cached content no longer matches the installed digest", async () => {
    ...
    const trustedDiscovery = await discoverVykorExtensions(cwd, BASE_SETTINGS);
    expect(trustedDiscovery.plugins.map((plugin) => plugin.manifest.id)).toEqual(["dev.vykor.digest-check"]);
    ...
    expect(discovery.plugins).toEqual([]);
    expect(discovery.warnings).toEqual([
      "dev.vykor.digest-check: cached plugin content does not match the installed digest; reinstall the plugin",
    ]);
    ...
    const recoveredDiscovery = await discoverVykorExtensions(cwd, BASE_SETTINGS);
    expect(recoveredDiscovery.plugins.map((plugin) => plugin.manifest.id)).toEqual(["dev.vykor.digest-check"]);
  ...
  it("rejects a legacy copied user installation that has no trusted digest", async () => {
    ...
    expect(discovery.plugins).toEqual([]);
    expect(discovery.warnings).toEqual([
      "dev.vykor.runtime-tool: copied plugin installation has no trusted content digest; reinstall the plugin",
    ]);
  });
  ...
  it("rejects a copied plugin whose installed snapshot root was replaced by a directory link", async () => {
    ...
    expect(discovery.plugins).toEqual([]);
    expect(discovery.warnings).toEqual([
      "dev.vykor.linked-snapshot: copied plugin cache snapshot is not a regular directory; reinstall the plugin",
    ]);
  });

  it.each([
    ["id", "dev.vykor.replaced", "1.0.0"],
    ["version", "dev.vykor.runtime-tool", "2.0.0"],
    ...
    expect(discovery.plugins).toEqual([]);
    expect(discovery.warnings).toEqual([
      `dev.vykor.runtime-tool: actual plugin identity ${id}@${version} differs from installed identity dev.vykor.runtime-tool@1.0.0; reinstall the plugin`,
    ]);
  });
  ...
  it("allows linked plugin code to change when its manifest identity and permissions stay unchanged", async () => {
    ...
    expect(discovery.plugins.map((plugin) => plugin.manifest.id)).toEqual(["dev.vykor.runtime-tool"]);
    expect(discovery.warnings).toEqual([]);
  });
  ...
  it("discovers, invokes, and removes a Tool with its owning runtime", async () => {
    ...
    writeProjectToolPlugin(cwd);
    const discovery = await discoverVykorExtensions(cwd, BASE_SETTINGS);
    expect(discovery.pluginCapabilityInventory.plugins.get("dev.vykor.runtime-tool"))
      .toMatchObject({
        pluginId: "dev.vykor.runtime-tool",
        version: "1.0.0",
        scope: "user",
        origin: "native",
        nativeToolEntries: ["plugin:dev.vykor.runtime-tool:tool:./tools/index.mjs"],
      });
    expect(discovery.pluginCapabilityInventory.nativeToolEntries.get(
      "plugin:dev.vykor.runtime-tool:tool:./tools/index.mjs",
    ))
      .toEqual({ pluginId: "dev.vykor.runtime-tool" });
    ...
    expect(runtime.toolRegistry.inspect("InstalledPluginEcho")).toEqual({
      ...
      source: { kind: "plugin", id: "dev.vykor.runtime-tool" },
    });
