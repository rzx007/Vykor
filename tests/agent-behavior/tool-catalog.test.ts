...
function catalog(size: number): ToolDefinition[] {
  return Array.from({ length: size }, (_, index) => ({
    ...
    execute: async () => ({ content: [{ type: "text" as const, text: "evidence" }] }),
  }));
}
...
function scenario(size: number): BehaviorCase {
  return { id: `catalog-${size}`, domain: "research", prompt: "Inspect the first two sources", setup: () => ({
    ...
    verify: () => ({ passed: true, reason: "catalog observed" }),
  }) };
}
...
describe("tool catalog measurement", () => {
  it.each([4, 8, 40, 120])("measures the %i-tool catalog actually sent to the provider", async (size) => {
    ...
    expect(sent[0]?.map((tool) => tool.name)).toEqual(catalog(size).slice(0, -1).map((tool) => tool.name));
    const serializedLength = JSON.stringify(sent[0]).length;
    expect(result.toolCatalogRequests).toEqual([{
    ...
    expect(result.estimatedToolTokens).toBe(Math.ceil(serializedLength / 4));
    expect(result.actualInputTokens).toBe(10_000);
    if (size === 40) {
      ...
      expect(sent[1]?.map((tool) => tool.name)).toEqual(sent[0]?.map((tool) => tool.name));
    }
  });
  ...
  it("records definition changes across compaction and counts no definitions for a summary request", async () => {
    ...
    expect(result.status).toBe("passed");
    expect(result.toolCatalogRequests!.map((request) => request.definitionCount)).toContain(0);
    const summary = result.toolCatalogRequests!.find((request) => request.definitionCount === 0);
